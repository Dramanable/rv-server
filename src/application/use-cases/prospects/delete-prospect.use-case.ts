...
export class DeleteProspectUseCase {
   ...
   */
  private validateBusinessRulesForDeletion(prospect: Prospect): void {
    ...
    if (daysSinceLastUpdate < 7 && prospect.getStatus().isActive()) {
      throw new ProspectValidationError(
        'Cannot delete prospects with recent activity. Wait 7 days or archive instead.',
      );
    }
