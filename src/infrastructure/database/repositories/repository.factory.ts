...
export class RepositoryFactory {
  ...
  private getDatabaseTypeFor(repositoryType: RepositoryType): DatabaseType {
    return (
      this.config.overrides?.[repositoryType] || this.config.defaultDatabaseType
    );
  }
  ...
  private loadConfiguration(): RepositoryFactoryConfig {
    ...
    return {
      ...
      overrides: {
        ...
        user: this.configService.get<DatabaseType>(
          'USER_REPOSITORY_TYPE',
          defaultType,
        ),
        business: this.configService.get<DatabaseType>(
          'BUSINESS_REPOSITORY_TYPE',
          defaultType,
        ),
        calendar: this.configService.get<DatabaseType>(
          'CALENDAR_REPOSITORY_TYPE',
          defaultType,
        ),
  ...
  private createSqlUserRepository(): UserRepository {
    ...
    throw new Error(
      'SQL UserRepository creation not implemented - requires TypeORM dependencies injection',
    );
  }
  ...
  private createSqlBusinessRepository(): BusinessRepository {
    ...
    // TODO: Injecter les dépendances TypeORM appropriées
    throw new Error(
      'SQL BusinessRepository creation not implemented - requires TypeORM dependencies injection',
    );
  }
  ...
  private createSqlCalendarRepository(): CalendarRepository {
    ...
    // TODO: Injecter les dépendances TypeORM appropriées
    throw new Error(
      'SQL CalendarRepository creation not implemented - requires TypeORM dependencies injection',
    );
  }
  ...
  private createNoSqlUserRepository(): UserRepository {
    ...
    throw new Error(
      'NoSQL UserRepository creation not implemented - requires Mongoose dependencies injection',
    );
  }
  ...
  private createNoSqlBusinessRepository(): BusinessRepository {
    ...
    // TODO: Injecter les dépendances Mongoose appropriées
    throw new Error(
      'NoSQL BusinessRepository creation not implemented - requires Mongoose dependencies injection',
    );
  }
  ...
  private createNoSqlCalendarRepository(): CalendarRepository {
    ...
    // TODO: Injecter les dépendances Mongoose appropriées
    throw new Error(
      'NoSQL CalendarRepository creation not implemented - requires Mongoose dependencies injection',
    );
  }
...
PRODUCTION_DATABASE_TYPE=nosql
  ...
  static validateConfig(config: RepositoryFactoryConfig): string[] {
    ...
    if (!['sql', 'nosql'].includes(config.defaultDatabaseType)) {
      errors.push(
        `Invalid default database type: ${config.defaultDatabaseType}`,
      );
    }
    ...
    const usesSql =
      config.defaultDatabaseType === 'sql' ||
