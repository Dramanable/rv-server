...
describe('RoleUtils - Advanced Permission System', () => {
  ...
  describe('Permission Checks', () => {
    it('should correctly identify platform admin permissions', () => {
      expect(RoleUtils.hasPermission(UserRole.PLATFORM_ADMIN, Permission.MANAGE_SYSTEM_SETTINGS)).toBe(true);
      expect(RoleUtils.hasPermission(UserRole.PLATFORM_ADMIN, Permission.CONFIGURE_BUSINESS_SETTINGS)).toBe(true);
      expect(RoleUtils.hasPermission(UserRole.PLATFORM_ADMIN, Permission.BOOK_APPOINTMENT)).toBe(true);
    });

    it('should correctly identify business owner permissions', () => {
      expect(RoleUtils.hasPermission(UserRole.BUSINESS_OWNER, Permission.CONFIGURE_BUSINESS_SETTINGS)).toBe(true);
      expect(RoleUtils.hasPermission(UserRole.BUSINESS_OWNER, Permission.MANAGE_ALL_STAFF)).toBe(true);
      expect(RoleUtils.hasPermission(UserRole.BUSINESS_OWNER, Permission.MANAGE_SYSTEM_SETTINGS)).toBe(false);
    });

    it('should correctly identify practitioner permissions', () => {
      expect(RoleUtils.hasPermission(UserRole.PRACTITIONER, Permission.MANAGE_OWN_SCHEDULE)).toBe(true);
      expect(RoleUtils.hasPermission(UserRole.PRACTITIONER, Permission.VIEW_CLIENT_HISTORY)).toBe(true);
      expect(RoleUtils.hasPermission(UserRole.PRACTITIONER, Permission.MANAGE_ALL_STAFF)).toBe(false);
    });

    it('should correctly identify client permissions', () => {
      expect(RoleUtils.hasPermission(UserRole.REGULAR_CLIENT, Permission.BOOK_APPOINTMENT)).toBe(true);
      expect(RoleUtils.hasPermission(UserRole.REGULAR_CLIENT, Permission.VIEW_OWN_APPOINTMENTS)).toBe(true);
      expect(RoleUtils.hasPermission(UserRole.REGULAR_CLIENT, Permission.MANAGE_OWN_SCHEDULE)).toBe(false);
    });
  });
  ...
  describe('Role Hierarchy', () => {
    ...
    it('should allow higher roles to act on lower roles', () => {
      expect(RoleUtils.canActOnRole(UserRole.BUSINESS_OWNER, UserRole.PRACTITIONER)).toBe(true);
      expect(RoleUtils.canActOnRole(UserRole.PRACTITIONER, UserRole.BUSINESS_OWNER)).toBe(false);
      expect(RoleUtils.canActOnRole(UserRole.PRACTITIONER, UserRole.REGULAR_CLIENT)).toBe(true);
    });
  });
  ...
  describe('Role Groups', () => {
    ...
    it('should correctly identify practitioner roles', () => {
      expect(RoleUtils.isPractitionerRole(UserRole.SENIOR_PRACTITIONER)).toBe(true);
      expect(RoleUtils.isPractitionerRole(UserRole.PRACTITIONER)).toBe(true);
      expect(RoleUtils.isPractitionerRole(UserRole.JUNIOR_PRACTITIONER)).toBe(true);
  ...
  describe('Business Type Permissions', () => {
    it('should provide correct permissions for medical clinic', () => {
      const medicalPermissions = RoleUtils.getBusinessTypePermissions(BusinessType.MEDICAL_CLINIC);
    ...
    it('should provide correct permissions for law firm', () => {
      const lawPermissions = RoleUtils.getBusinessTypePermissions(BusinessType.LAW_FIRM);
  ...
  describe('Role Assignment', () => {
    it('should return assignable roles for business owner', () => {
      const assignableRoles = RoleUtils.getAssignableRoles(UserRole.BUSINESS_OWNER);
    ...
    it('should return limited assignable roles for practitioner', () => {
      const assignableRoles = RoleUtils.getAssignableRoles(UserRole.PRACTITIONER);
    ...
    it('should return limited assignable roles for clients', () => {
      const assignableRoles = RoleUtils.getAssignableRoles(UserRole.REGULAR_CLIENT);
  ...
  describe('Role Descriptions', () => {
    it('should provide meaningful descriptions for all roles', () => {
      const platformAdminDesc = RoleUtils.getRoleDescription(UserRole.PLATFORM_ADMIN);
      expect(platformAdminDesc).toContain('Plateforme');
      expect(platformAdminDesc).toContain('multi-tenant');

      const practitionerDesc = RoleUtils.getRoleDescription(UserRole.PRACTITIONER);
  ...
  describe('Edge Cases', () => {
    it('should handle invalid roles gracefully', () => {
      expect(RoleUtils.hasPermission('INVALID_ROLE' as UserRole, Permission.BOOK_APPOINTMENT)).toBe(false);
      expect(RoleUtils.getRoleLevel('INVALID_ROLE' as UserRole)).toBe(0);
      expect(RoleUtils.getRoleDescription('INVALID_ROLE' as UserRole)).toBe('Rôle non défini');
    });

    it('should handle empty permission arrays', () => {
      const emptyPermissions = RoleUtils.getRolePermissions('INVALID_ROLE' as UserRole);
