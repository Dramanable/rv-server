 ...
 */
export const STAFF_TO_USER_ROLE_MAPPING: Record<StaffRole, import('./user-role.enum').UserRole> = {
  // Direction
...
export class StaffRoleUtils {
  ...
  static getSeniorityLevel(staffRole: StaffRole): SeniorityLevel {
    ...
    if ([StaffRole.SITE_MANAGER, StaffRole.DEPARTMENT_HEAD].includes(staffRole)) {
    ...
    if (staffRole.includes('JUNIOR_') || staffRole.includes('STUDENT') || 
        staffRole === StaffRole.INTERN || staffRole === StaffRole.RESIDENT) {
  ...
  static requiresLicense(staffRole: StaffRole): boolean {
    const licensedRoles = [
      ...
      StaffRole.SENIOR_DENTIST, StaffRole.DENTIST,
      StaffRole.SENIOR_LAWYER, StaffRole.LAWYER,
      StaffRole.HYGIENIST, StaffRole.NURSE,
      StaffRole.SENIOR_THERAPIST, StaffRole.THERAPIST,
    ];
  ...
  static getDescription(staffRole: StaffRole): string {
    const descriptions: Record<StaffRole, string> = {
      ...
      [StaffRole.SITE_MANAGER]: 'Gestionnaire de Site - Supervision opérationnelle d\'un lieu',
      [StaffRole.DEPARTMENT_HEAD]: 'Chef de Département - Direction d\'une spécialité',
      [StaffRole.TEAM_LEAD]: 'Chef d\'Équipe - Leadership d\'une équipe spécialisée',
      
      [StaffRole.SENIOR_DOCTOR]: 'Médecin Senior - Expert médical avec mentorat',
      [StaffRole.DOCTOR]: 'Médecin - Praticien médical certifié',
      [StaffRole.RESIDENT]: 'Résident - Médecin en spécialisation',
      
      [StaffRole.SENIOR_DENTIST]: 'Dentiste Senior - Expert dentaire avec mentorat',
      [StaffRole.DENTIST]: 'Dentiste - Praticien dentaire certifié',
      [StaffRole.DENTAL_STUDENT]: 'Étudiant Dentaire - En formation dentaire',
      
      [StaffRole.SENIOR_LAWYER]: 'Avocat Senior - Expert juridique avec mentorat',
      [StaffRole.LAWYER]: 'Avocat - Praticien juridique certifié',
      [StaffRole.PARALEGAL]: 'Assistant Juridique - Support juridique spécialisé',
      
      [StaffRole.SENIOR_THERAPIST]: 'Thérapeute Senior - Expert thérapeutique avec mentorat',
      [StaffRole.THERAPIST]: 'Thérapeute - Praticien thérapeutique certifié',
      [StaffRole.THERAPY_INTERN]: 'Stagiaire Thérapie - En formation thérapeutique',
      
      [StaffRole.SENIOR_STYLIST]: 'Styliste Senior - Expert beauté avec mentorat',
      [StaffRole.STYLIST]: 'Styliste - Praticien beauté certifié',
      [StaffRole.JUNIOR_STYLIST]: 'Styliste Junior - En développement beauté',
      
      [StaffRole.SENIOR_CONSULTANT]: 'Consultant Senior - Expert conseil avec mentorat',
      [StaffRole.CONSULTANT]: 'Consultant - Praticien conseil certifié',
      [StaffRole.JUNIOR_CONSULTANT]: 'Consultant Junior - En développement conseil',
      
      [StaffRole.HEAD_RECEPTIONIST]: 'Chef Réceptionniste - Supervision de l\'accueil',
      [StaffRole.RECEPTIONIST]: 'Réceptionniste - Accueil et gestion front office',
      [StaffRole.SENIOR_ASSISTANT]: 'Assistant Senior - Support administratif expérimenté',
      [StaffRole.ASSISTANT]: 'Assistant - Support administratif général',
      [StaffRole.APPOINTMENT_COORDINATOR]: 'Coordinateur RDV - Spécialiste planification',
