...
describe('BusinessName', () => {
  describe('create', () => {
    ...
    it('should throw error for empty name', () => {
      expect(() => BusinessName.create('')).toThrow('Business name cannot be empty');
    });
    ...
    it('should throw error for name too short', () => {
      expect(() => BusinessName.create('A')).toThrow('Business name must be at least 2 characters long');
    });

    it('should throw error for name too long', () => {
      const longName = 'A'.repeat(101);
      expect(() => BusinessName.create(longName)).toThrow('Business name cannot exceed 100 characters');
    });

    it('should throw error for forbidden characters', () => {
      expect(() => BusinessName.create('Business<Name')).toThrow('Business name contains forbidden characters');
      expect(() => BusinessName.create('Business>Name')).toThrow('Business name contains forbidden characters');
      expect(() => BusinessName.create('Business{Name')).toThrow('Business name contains forbidden characters');
      expect(() => BusinessName.create('Business}Name')).toThrow('Business name contains forbidden characters');
      expect(() => BusinessName.create('Business[Name')).toThrow('Business name contains forbidden characters');
