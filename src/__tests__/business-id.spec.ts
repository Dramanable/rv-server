...
describe('BusinessId', () => {
  describe('create', () => {
    ...
    it('should throw error for invalid UUID format', () => {
      expect(() => BusinessId.create('invalid-uuid')).toThrow('BusinessId must be a valid UUID v4');
      expect(() => BusinessId.create('123456789')).toThrow('BusinessId must be a valid UUID v4');
      expect(() => BusinessId.create('not-a-uuid-at-all')).toThrow('BusinessId must be a valid UUID v4');
    });

    it('should throw error for UUID v1 (not v4)', () => {
      const uuidV1 = '550e8400-e29b-11d4-a716-446655440000'; // UUID v1
      expect(() => BusinessId.create(uuidV1)).toThrow('BusinessId must be a valid UUID v4');
    });
